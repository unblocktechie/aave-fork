...
import { ERC_20_ABI } from '../interfaces/ERC20';
import { IS_MAINNET } from '../config/Environment';
import { normalizeAvg } from '../helpers';
...
export default class RatesHistory implements RatesHistoryInterface {
  ...
  private readonly getRatesBetween = async (
    ...
    const {
      utilizationRate,
      stableBorrowRate: stableBorrowRateRaw,
      availableLiquidity: availableLiquidityRaw,
      totalATokenSupply: totalATokenSupplyRaw,
    } = from;
    const {
      availableLiquidity: availableLiquidityRawTo,
      totalATokenSupply: totalATokenSupplyRawTo,
    } = to;
    const provider = new ethers.providers.JsonRpcProvider(
      IS_MAINNET
        ? 'https://polygon-rpc.com'
        : 'https://rpc-mumbai.maticvigil.com/',
      IS_MAINNET ? 137 : 80001,
    );
    const tokenContract = new ethers.Contract(
      from.reserve.id.slice(0, 42),
      ERC_20_ABI,
      provider,
    );
    const tokenDecimals = await tokenContract.decimals();
    const stableBorrowRate = normalize(stableBorrowRateRaw, 27);
    const availableLiquidity = normalizeAvg(
      availableLiquidityRaw,
      availableLiquidityRawTo,
      tokenDecimals,
    );
    const totalATokenSupply = normalizeAvg(
      totalATokenSupplyRaw,
      totalATokenSupplyRawTo,
      tokenDecimals,
    );
  ...
  private readonly getRates = async (
    ...
    for (let i = 0; i < reservesHistory.length - 1; i += 1) {
      if (reservesHistory[i + 1].timestamp <= currentTimestamp) {
        ...
        if (!to) break;
        const rates = await this.getRatesBetween(
          { from, to },
          currentTimestamp,
        );
        if (rates.length) {
  ...
  private readonly updateRates = async (): Promise<void> => {
    ...
    for (const reserveId of reserveIds) {
      ...
      const rateEntries = await this.getRates(
  ...
  public getRatesHistory = async (
    ...
    if (!reserveId) {
      res.status(500).send({
        error: 'You need to provide a reserveId',
      });
      return;
    }
    try {
