...
export const ModalWrapper: React.FC<{
  ...
  return (
    <AssetCapsProvider asset={poolReserve}>
      {!mainTxState.success && isTitleShown && (
      ...
      {headerNavList && (
        <>
          <ul className={css.header}>
            {headerNavList.map((item, i) => (
              <li
                className={cn(
                  css.headerItem,
                  item.active && css.headerItemActive,
                  item.isDisabled && css.headerItemDisabled,
                  item.isSecondaryColors && css.headerItemSecondary
                )}
          ...
          <div className={css.tokenWrapper}>
            <img
              src={`/icons/tokens/${(
                headerNavList[0]?.icon || poolReserve.iconSymbol
              ).toLowerCase()}.svg`}
              alt="token-icon"
          ...
          <span
            className={cn(
              css.tokenLabel,
              headerNavList[0].isSecondaryColors && css.tokenLabelSecondary
            )}
          >
            {headerNavList[0]?.icon || poolReserve.iconSymbol}
          </span>
