import { API_ETH_MOCK_ADDRESS, InterestRate, PERMISSION } from '@aave/contract-helpers';
...
export const RepayModal = () => {
  ...
  const { openBorrow, openBorrowModalData } = useModalContext();
  ...
  const headerNavList = [
    {
      text: 'Borrow',
      onClick: () => {
        if (openBorrowModalData) {
          const { funnel, currentMarket, name, underlyingAsset } = openBorrowModalData;
          openBorrow(underlyingAsset, currentMarket, name, funnel);
        }
      },
      isDisabled: !openBorrowModalData?.active,
      active: false,
      isSecondaryColors: true,
      icon:
        openBorrowModalData?.underlyingAsset.toLowerCase() === API_ETH_MOCK_ADDRESS.toLowerCase()
